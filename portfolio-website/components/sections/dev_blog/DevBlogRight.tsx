...
export default function DevBlogRight({ selectedId, hoveredId }: DevBlogRightProps) {
  ...
  const postMeta = (
    <div className="flex items-center gap-3 mb-[28px] flex-wrap">
      ...
      {post.readTime && <span className="text-[12px] text-gray-400">{post.readTime}</span>}
    </div>
  );
  ...
  if (!post.body) {
    return (
      <div className="flex flex-col h-full">
        {postMeta}
        <div className="text-[26px] lg:text-[40px] font-bold text-gray-900 tracking-[-0.02em] leading-[1.15] mb-[28px]">
          {post.title}
        </div>
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div
        ...
        // Sanitize before rendering raw HTML to prevent XSS
